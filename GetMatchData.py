...
todays_date = datetime.datetime(2022, 1, 15).strftime('%Y-%m-%d')
...
filt = df.kickoff_date == todays_date
...
response = sqs.send_message(
    ...
    MessageAttributes={
        'Title': {
            'DataType': 'String',
            'StringValue': 'FPL Gameday Finished'
        }
    },
    MessageBody=(
        'The Last game today is at ' + dftime
    )
)
